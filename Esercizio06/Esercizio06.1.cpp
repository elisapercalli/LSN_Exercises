...
int main()
{
  ...
  for(int i=0; i<500; i++)  //Equilibration
    Move(metro);
...
//Read input informations
  ...
  if(redo==false) cout <<"The program start from a random spin configuration"<<endl;
  else cout <<"The program start from a frevious spin configuartion"<<endl;
  ReadInput.close();
...
{
  int o, sm;
  double p, energy_old, energy_new, A;
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    stima_c=beta*beta*((blk_av[ic]/blk_norm)-pow(stima_u,2)); //Heat capacity
    glob_av[ic]  += stima_c;
    ...
    stima_x=beta*blk_av[ix]/blk_norm; //Magnetic susceptibility
    glob_av[ix]  += stima_x;
