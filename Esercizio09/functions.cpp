...
void population :: start(){
  ...
  for (int i=1; i<n_element; i++){ //genero altri elementi permutando casualmente dal precedente
    element one(cities);
    one.copy(gen[i-1]);
...
//pesco elementi nella vecchia generazione con roulette truccata, tento di mutarli
  new_gen[0].copy(gen[0]); //salvo elemento migliore nell nuova gen
  //salvo un altro elemento casuale , che provo a mutare (mi serve per avere le coppie pari)
  int o=select();
  one.copy(gen[o]);
  if (rnd->Rannyu()<r_mut){
    mutate(o);
  ...
  new_gen[1].copy(gen[o]);
  gen[0].copy(one);
  for (int i=2; i<n_element; i+=2){      //ciclo sulle coppie di genitori
    k=select();
    one.copy(gen[k]);
    if (rnd->Rannyu()<r_mut){
      mutate(k);
...
//tento di crossare i due elementi selezionati
    if (rnd->Rannyu()<r_cross){
      crossover(l,k);
    }
    new_gen[i]=gen[k];      //salvo elementi crossati nella nuova generazione
    new_gen[i+1]=gen[l];
...
void population :: PrintBest(char * filename){
  ofstream fout(filename);
  int j=0;
  if(!fout) {
    cout<<"cannot open output_file"<<endl;
