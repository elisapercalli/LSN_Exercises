...
double variationalWaveFunction(double v){
	//return 1.0;
	  double ep, em, mu=0.795, sigma=0.62;
    ep=exp(-pow(v+mu,2)/(2*sigma*sigma));
    em=exp(-pow(v-mu,2)/(2*sigma*sigma));
    return ep+em;
}
...
double variationalWaveFunction_second(double v){
    ...
    ep=exp(-pow(v+mu,2)/(2*sigma*sigma));
    em=exp(-pow(v-mu,2)/(2*sigma*sigma));
    return -(em*(1-pow((v-mu)/sigma,2))+ep*(1-pow((v+mu)/sigma,2)))/pow(sigma,2);
}
