...
//prepare for mesurements of g(r)
  nbins = 100;
...
void MeasureAve(){
  ...
  for (int i=0; i<nbins; i++){
    r=i*bin_size;
    V=4*M_PI/3*(pow(r+bin_size,3)-pow(r,3));
    gdir=blk_av[i]/L/rho/npart/V;
    glob_av[i]+=gdir;
...
void Gofr(int n){
  ofstream Gofr;
  const int wd=12;
  Gofr.open("NVE.gave.0");
  for(int i=0; i<nbins; i++){
    err_gdir=Error(glob_av[i],glob_av2[i],n);
    Gofr << setw(wd) << glob_av[i]/(double)n << setw(wd) << err_gdir << endl;
  }
  Gofr.close();
}
