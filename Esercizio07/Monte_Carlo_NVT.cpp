...
// contribution to energy and virial
  ...
  Write.open("output.instant", ios::app);
  Write<< setw(wd)<< walker[iv]/(double)npart + vtail<<"\t"<<setw(wd)<<rho * temp + (walker[iw] + ptail * (double)npart) / vol<<endl;
...
//g(r)
    for (int i=0; i<nbins; i++){
      r=i*bin_size;
      V=4*pi/3*(pow(r+bin_size,3)-pow(r,3));
      gdir=blk_av[igofr+i]/blk_norm/rho/npart/V;
    ...
    if (iblk==nblk){
      Gofr.open("output.gave.0");
      for(int i=0; i<nbins; i++){
        err_gdir=Error(glob_av[igofr+i],glob_av2[igofr+i],iblk);
        Gofr << setw(wd) << glob_av[igofr+i]/(double)iblk << setw(wd) << err_gdir << endl;
      }
      Gofr.close();
    }
}
